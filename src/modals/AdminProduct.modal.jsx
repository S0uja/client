...
const ProductsModal = (props) => {
    const ProductModalStatus=useSelector(state => state.modals.adminProductModal)
    ...
    const validate = () => {
        ...
        if (!fields.images.length) {
            setFieldsErrors(prevState => ({
                ...prevState,
                images: { status: true, message: '* Необходимо загрузить хотя бы одно изображение' }
            }))
            errors = true
        }
        else {
    ...
    const create = (event) => {
        ...
        if(!valid){
            setLoading(true)
            createProduct(fields.name,fields.price,fields.amount,fields.description,fields.expirationdate,fields.storageconditions,fields.structure,fields.weight_volume,fields.categoryId,fields.manufacturerId,fields.images.map((image)=>image.file)).then((res)=>{
                if(!res){
                    dispatch(setSnackbarModal({
                        modal:true,
                        severity:'error',
    ...
    const update = (event) => {
        ...
        if(!valid){
            setLoading(true)
            updateProduct(props.updateId,fields.name,fields.price,fields.amount,fields.description,fields.expirationdate,fields.storageconditions,fields.structure,fields.weight_volume,fields.categoryId,fields.manufacturerId,fields.images.map((image)=>image.file)).then((res)=>{
                if(!res){
                    dispatch(setSnackbarModal({
                        modal:true,
                        severity:'error',
            ...
            <Box sx={{...modal,minHeight:'450px',maxWidth:'350px',justifyContent:'start',flexDirection:'column'}}>
...

                {
                    ...
                    <Box sx={{width:'100%',height:'100%'}}>
                        <Box sx={{...font,fontSize:'22px',mt:2,mb:3,textAlign:'center'}}>{mode==='create'?'Новый товар':'Редактирование'}</Box>
