...
import {createCollection,getCollection,updateCollection} from '../http/Admin.http'
...
const CollectionModal = (props) => {
    const CollectionModalStatus=useSelector(state => state.modals.adminCollectionModal)
    ...
    useEffect(()=>{
        ...
        getCollection(props.updateId).then(async res => {
            if(!res){
                ...
                const products = res.data.data[0].collection_products.map((product) => product.product)
                setFields({
    ...
    const create = (event) => {
        ...
        if(!valid){
            setLoading(true)
            createCollection(fields.name,fields.visible,fields.products.map((product)=>product.id)).then((res)=>{
                if(!res){
                    dispatch(setSnackbarModal({
                        modal:true,
                        severity:'error',
    ...
    const update = (event) => {
        ...
        if(!valid){
            setLoading(true)
            updateCollection(props.updateId,fields.name,fields.visible,fields.products.map((product)=>product.id)).then((res)=>{
                if(!res){
                    dispatch(setSnackbarModal({
                        modal:true,
                        severity:'error',
            ...
            <Box sx={{...modal,minHeight:'450px',maxWidth:'350px',justifyContent:'start',flexDirection:'column'}}>
...

                {
                    ...
                    <Box sx={{width:'100%',height:'100%'}}>
                        <Box sx={{...font,fontSize:'22px',mt:2,mb:3,textAlign:'center'}}>{mode==='create'?'Новая коллекция':'Редактирование'}</Box>
