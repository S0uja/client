...
import {createOrderStatus,getOrderStatus,updateOrderStatus} from '../http/Admin.http'

const OrderStatusModal = (props) => {
    const OrderStatusModalStatus=useSelector(state => state.modals.adminOrderStatusModal)
    ...
    useEffect(()=>{
        ...
        setMode('update')
        getOrderStatus(props.updateId).then((res)=>{
            if(!res){
    ...
    const create = (event) => {
        ...
        if(!valid){
            setLoading(true)
            createOrderStatus(fields.name).then((res)=>{
                if(!res){
                    dispatch(setSnackbarModal({
                        modal:true,
                        severity:'error',
    ...
    const update = (event) => {
        ...
        if(!valid){
            setLoading(true)
            updateOrderStatus(props.updateId,fields.name).then((res)=>{
                if(!res){
                    dispatch(setSnackbarModal({
                        modal:true,
                        severity:'error',
    ...
    return (
        ...
        >
            <Box sx={{...modal,height:'',minHeight:'200px',maxWidth:'350px',justifyContent:'start',flexDirection:'column'}}>
...

                {
                    ...
                    <Box sx={{width:'100%',height:'100%'}}>
                        <Box sx={{...font,fontSize:'22px',my:2,textAlign:'center'}}>{mode==='create'?'Новый статус заказа':'Редактирование'}</Box>
