...
import {createUser,getUser} from '../http/Admin.http'
import { updateUser } from '../http/Admin.http'

const PersonalModal = (props) => {
    const PersonalModalStatus=useSelector(state => state.modals.adminPersonalModal)
    ...
    const update = (event) => {
        ...
        if(!valid){
            setLoading(true)
            updateUser(props.updateId,fields.number,fields.fio,fields.role,fields.birthdate).then((res)=>{
                if(!res){
                    dispatch(setSnackbarModal({
                        modal:true,
                        severity:'error',
            ...
            <Box sx={{...modal,height:'100%',minHeight:'450px',maxWidth:'350px',justifyContent:'start',flexDirection:'column'}}>
                ...
                </IconButton>
                {
                    ...
                    <Box sx={{width:'100%',height:'100%'}}>
                        <Box sx={{...font,textAlign:'center',fontSize:'22px',mt:2,mb:3}}>{mode==='create'?'Новый сотрудник':'Редактирование'}</Box>
