...
const OrderModal = (props) => {
    ...
    useEffect(()=>{
        ...
        getOrder(props.updateId).then(async res => {
            if(!res){
                ...
                const products = res.data.data[0].order_products.map((product) => {return {...product.product,count:product.count}})
                const user = {id: res.data.data[0].user.id,name: res.data.data[0].user.number,fio: res.data.data[0].user.fio}
                setFields({
    ...
    const calcPrice = (products,mk=false) => {
        ...
        setFields(prevState => ({
            ...prevState,
            price: products.reduce((acc,product)=>{return acc+product.price*product.count},0)
        }))
    }
    ...
    const create = (event) => {
        ...
        if(!valid){
            ...
            createOrder(fields.address,fields.products.map((product)=>{return {id:product.id,count:product.count}}), fields.price, fields.user.id, fields.status.id).then((res)=>{
                if(!res){
                    dispatch(setSnackbarModal({
                        modal:true,
                        severity:'error',
    ...
    const update = (event) => {
        ...
        if(!valid){
            setLoading(true)
            updateOrder(props.updateId,fields.address,fields.products.map((product)=>{return {id:product.id,count:product.count}}), fields.price, fields.user.id, fields.status.id).then((res)=>{
                if(!res){
                    dispatch(setSnackbarModal({
                        modal:true,
                        severity:'error',
            ...
            <Box sx={{...modal,minHeight:'450px',maxWidth:'350px',justifyContent:'start',flexDirection:'column'}}>
...

                {
                    ...
                    <Box sx={{width:'100%',height:'100%'}}>
                        <Box sx={{...font,fontSize:'22px',mt:2,mb:3,textAlign:'center'}}>{mode==='create'?'Новый заказ':'Редактирование'}</Box>
