...
import { getOrderStatuses,deleteOrder,getProducts,getUsers, getOrders } from '../http/Admin.http';
...
const OrdersComponent = () => {
    ...
    const updateData = () => {
        getOrders().then(res=>{
            if(!res){
                ...
                const data=res.data.data.map(item=>{
                    const products = item.order_products.map(product=>product.product.name).join(', ')
                    return {
        ...
        getProducts().then(res => {
            if(!res){
                ...
                const data = res.data.data.map(product =>{return {...product,count:1}})
                setProducts(data)
            }
            else{
                dispatch(setSnackbarModal({
    ...
    return (
        <>
            <OrderModal order_statuses={orderStatuses} users={users} products={products} handleUpdate={updateData} updateId={updateId}/>
            
            <Paper sx={{height:'100%', width: '100%', overflow: 'hidden',p:2,borderRadius:2,boxSizing:'border-box'}} elevation={0}>
                <Typography sx={{...font,fontSize:'24px',width:'100%'}}>Заказы</Typography>
